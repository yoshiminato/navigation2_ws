...
class Nav2CombinedExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space, features_dim=256):
        ...
        # 定数の定義（環境側の設定に合わせてください）
        self.img_h, self.img_w = 30, 30
        self.n_stack_frames = 4  # スタックするCostmapの枚数

        self.img_size = self.img_h * self.img_w * self.n_stack_frames  # 3600
        ...
        self.cnn = nn.Sequential(
            # 入力: (Batch, 4, 30, 30)
            # 1層目: 4フレームを統合して特徴抽出
            nn.Conv2d(self.n_stack_frames, 32, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2), # 30x30 -> 15x15
            
            # 2層目: より抽象的な特徴(角や通路の形状)を抽出
            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2), # 15x15 -> 7x7
            
            # 3層目: 全体の位置関係を統合
            nn.Conv2d(64, 64, kernel_size=3, stride=1), # 7x7 -> 5x5
            nn.ReLU(),
            nn.Flatten(),
        )
